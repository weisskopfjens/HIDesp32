...
public:
  BleComboKeyboard(std::string deviceName = "Bluetooth Keyboard/Mouse", std::string deviceManufacturer = "Noname", uint8_t batteryLevel = 100);
  void begin(void);
  void end(void);
  void sendReport(KeyReport* keys);
  void sendReport(MediaKeyReport* keys);
  size_t pressraw(uint8_t k);
  size_t pressmod(uint8_t k);
  size_t press(uint8_t k);
  size_t press(const MediaKeyReport k);
  size_t releaseraw(uint8_t k);
  size_t release(uint8_t k);
  size_t release(const MediaKeyReport k);
  size_t writeraw(uint8_t c);
