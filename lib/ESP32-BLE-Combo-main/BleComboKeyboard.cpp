...
const uint8_t _asciimap[256] =
{
	...
	0x2d|SHIFT,    // ?
	0x14|ALT_GR,   // @
	0x04|SHIFT,    // Keyboard a and A
	0x05|SHIFT,    // Keyboard b and B
	0x06|SHIFT,    // Keyboard c and C
	0x07|SHIFT,    // Keyboard d and D
	0x08|SHIFT,    // Keyboard e and E
	0x09|SHIFT,    // Keyboard f and F	70
	0x0a|SHIFT,    // Keyboard g and G
	0x0b|SHIFT,    // Keyboard h and H
	0x0c|SHIFT,    // Keyboard i and I
	0x0d|SHIFT,    // Keyboard j and J
	0x0e|SHIFT,    // Keyboard k and K
	0x0f|SHIFT,    // Keyboard l and L
	0x10|SHIFT,    // Keyboard m and M
	0x11|SHIFT,    // Keyboard n and N
	0x12|SHIFT,    // Keyboard o and O
	0x13|SHIFT,    // Keyboard p and P	80
	0x14|SHIFT,    // Keyboard q and Q
	0x15|SHIFT,    // Keyboard r and R
	0x16|SHIFT,    // Keyboard s and S
	0x17|SHIFT,    // Keyboard t and T
	0x18|SHIFT,    // Keyboard u and U
	0x19|SHIFT,    // Keyboard v and V
	0x1a|SHIFT,    // Keyboard w and W
	0x1b|SHIFT,    // Keyboard x and X
	0x1c|SHIFT,    // Keyboard y and Y	
	0x1d|SHIFT,    // Keyboard z and Z	90
	0x25|ALT_GR,   // [
	0x2d|ALT_GR,   // bslash
	0x26|ALT_GR,   // ]
...
size_t BleComboKeyboard::pressraw(uint8_t k)
{
	uint8_t i;
...
// USB HID works, the host acts like the key remains pressed until we
// call release(), releaseAll(), or otherwise clear the report and resend.
size_t BleComboKeyboard::pressmod(uint8_t k)
{
	uint8_t i;
...
size_t BleComboKeyboard::releaseraw(uint8_t k)
{
	uint8_t i;
...
{
	uint8_t p = press(c);  // KeydownsendReport(&_keyReport);
	release(c);            // Keyup
	return p;              // just return the result of press() since release() almost always returns 1
}

size_t BleComboKeyboard::writeraw(uint8_t c)
{
	uint8_t p = pressraw(c);  // KeydownsendReport(&_keyReport);
	releaseraw(c);            // Keyup
	return p;              // just return the result of press() since release() almost always returns 1
}
...
size_t BleComboKeyboard::write(const uint8_t *buffer, size_t size) {
	size_t n = 0;
	while (size--) {
		if (*buffer != '\r') {sendReport(&_keyReport);
			if (write(*buffer)) {
			  n++;
			} else {
			  break;
